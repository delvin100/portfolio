        ...
        <CardContent>
          <form action={updateExperienceWithId} className="space-y-6">
            <div className="flex flex-col gap-2">
              ...
              <Input id="role" name="role" required defaultValue={experience.role} className="h-12 bg-muted/10 border-muted-foreground/20 focus-visible:ring-blue-500/50" />
            </div>

            <div className="flex flex-col gap-2">
              <label htmlFor="company" className="text-sm font-medium text-muted-foreground">Company</label>
              <Input id="company" name="company" required defaultValue={experience.company} className="h-12 bg-muted/10 border-muted-foreground/20 focus-visible:ring-blue-500/50" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                <label htmlFor="start_date" className="text-sm font-medium text-muted-foreground">Start Date</label>
                <Input id="start_date" name="start_date" type="date" defaultValue={experience.start_date || ''} className="h-12 bg-muted/10 border-muted-foreground/20 focus-visible:ring-blue-500/50" />
              </div>
              <div className="flex flex-col gap-2">
                ...
                <Input id="end_date" name="end_date" type="date" defaultValue={experience.end_date || ''} className="h-12 bg-muted/10 border-muted-foreground/20 focus-visible:ring-blue-500/50" />
              </div>
            </div>

            <div className="flex flex-col gap-2">
              <label htmlFor="description" className="text-sm font-medium text-muted-foreground">Description</label>
              <Textarea id="description" name="description" required defaultValue={experience.description} rows={5} className="bg-muted/10 border-muted-foreground/20 focus-visible:ring-blue-500/50 resize-none" />
            </div>

            <div className="flex flex-col gap-2">
              <label htmlFor="icon" className="text-sm font-medium text-muted-foreground">Icon Name (Lucide)</label>
              <Input id="icon" name="icon" defaultValue={experience.icon || 'Briefcase'} className="h-12 bg-muted/10 border-muted-foreground/20 focus-visible:ring-blue-500/50" />
